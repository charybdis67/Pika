...
class MyPokemonList extends Component {
                  ...
                  return (
                    <div key={own} className={classes.pokemonOwn}>
                      ...
                      <Button
                        variant="contained"
                        color="secondary"
                        onClick={() => this.favorPokemon(pokemon.name, own)}
                      >
                        Favor
                      </Button>
                    </div>
  ...
  favorPokemon = () => {
    const { favorPokemon } = this.props;

    favorPokemon();
  };

  favorPokemon = (name, own) => {
    const { favorPokemon, pokemons } = this.props;
    let favoredPokemon = pokemons.map(poke => {
      if (poke.name === name) {
    ...
    favoredPokemon = favoredPokemon.filter(poke => {
      return poke.owns.length > 0 ? true : false;
    });

    favorPokemon({ pokemons: favoredPokemon });
  };
  releaseAllPokemon = () => {
